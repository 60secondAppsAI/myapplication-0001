					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateApprovedBy()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2ApprovedBys = true">Add</base-button>
								              <modal :show.sync="modal2ApprovedBys">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Approved By Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="ApprovedById" type="text" placeholder="Enter ApprovedById" v-model="approvedByDetails.approvedById"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Approval Date</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<datepicker v-model="approvedByDetails.approvalDate"  placeholder="Select Date" ></datepicker>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updateApprovedBy() {
      ...
      const jsonData = JSON.stringify(this.approvedByDetails);
      const res = await ApprovedByService.update(jsonData);
...
//        this.getAllApprovedBys();
    ...
    async getApprovedByDetails() {
      try {
			let response = await ApprovedByService.get(this.approvedById);
			this.approvedByDetails = response.data;
