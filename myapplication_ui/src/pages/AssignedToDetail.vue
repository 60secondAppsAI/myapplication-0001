					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateAssignedTo()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2AssignedTos = true">Add</base-button>
								              <modal :show.sync="modal2AssignedTos">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Assigned To Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="AssignedToId" type="text" placeholder="Enter AssignedToId" v-model="assignedToDetails.assignedToId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Assignment Date</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<datepicker v-model="assignedToDetails.assignmentDate"  placeholder="Select Date" ></datepicker>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updateAssignedTo() {
      ...
      const jsonData = JSON.stringify(this.assignedToDetails);
      const res = await AssignedToService.update(jsonData);
...
//        this.getAllAssignedTos();
    ...
    async getAssignedToDetails() {
      try {
			let response = await AssignedToService.get(this.assignedToId);
			this.assignedToDetails = response.data;
