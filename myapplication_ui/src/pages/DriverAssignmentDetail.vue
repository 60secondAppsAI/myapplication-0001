					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateDriverAssignment()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2DriverAssignments = true">Add</base-button>
								              <modal :show.sync="modal2DriverAssignments">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Driver Assignment Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="DriverAssignmentId" type="text" placeholder="Enter DriverAssignmentId" v-model="driverAssignmentDetails.driverAssignmentId"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  data() {
    return {
      ...
      modal2DriverAssignments: false,
      driverAssignmentDetails: null,
  ...
  methods: {
    ...
    async handleAddSubmitted() {
      this.modal2DriverAssignments = false;
    ...
    async updateDriverAssignment() {
      ...
      const jsonData = JSON.stringify(this.driverAssignmentDetails);
      const res = await DriverAssignmentService.update(jsonData);
...
//        this.getAllDriverAssignments();
    ...
    async getDriverAssignmentDetails() {
      try {
			let response = await DriverAssignmentService.get(this.driverAssignmentId);
			this.driverAssignmentDetails = response.data;
 ...
 formatDate(dateString) {
  ...
  mounted() {
    this.getDriverAssignmentDetails();
  },
  computed: {
  },
