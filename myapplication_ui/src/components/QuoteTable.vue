        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalQuotes = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalQuotes">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Quote</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="QuoteId" type="text" placeholder="Enter QuoteId" v-model="quoteToAdd.quoteId"></base-input>
  <base-input label="DepartureDate" type="text" placeholder="Enter DepartureDate" v-model="quoteToAdd.departureDate"></base-input>
  <base-input label="Price" type="text" placeholder="Enter Price" v-model="quoteToAdd.price"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="quotes" :row-key="record => record.QuoteId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <QuotePictureView :quotes="quotes" />
          </div>
...
import QuotePictureView from './QuotePictureView.vue';
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'quoteId',
          visible: true,
          scopedSlots: { customRender: 'quoteId' },
          sorter: true
          //sorter: (a, b) => a.quoteId - b.quoteId,
      ...
      sortBy: 'quoteId',           // Column to sort by
  ...
  methods: {
    ...
    async renderQuotesTable() {
      ...
      for (let i = 0; i < this.quotes.length; i++) {
        quotesTableData.push({
          id: i,
          quoteId: this.quotes[i].quoteId,
          year: this.quotes[i].year,
          date: this.quotes[i].date,
          competitionId: this.quotes[i].competitionId,
          quoteId: this.quotes[i].quoteId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-quotes',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForQuotesChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalQuotes = false;

      const currentDate = new Date().getTime();
      this.quoteToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.quoteToAdd);
      console.log(jsonData);
      
      const res = await QuoteService.addQuote(jsonData);
