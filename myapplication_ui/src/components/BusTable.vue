        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Buss</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalBuss = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalBuss">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Bus</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="BusId" type="text" placeholder="Enter BusId" v-model="busToAdd.busId"></base-input>
  <base-input label="LicensePlate" type="text" placeholder="Enter LicensePlate" v-model="busToAdd.licensePlate"></base-input>
  <base-input label="Model" type="text" placeholder="Enter Model" v-model="busToAdd.model"></base-input>
  <base-input label="Seats" type="text" placeholder="Enter Seats" v-model="busToAdd.seats"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="buss" :row-key="record => record.BusId" :pagination="pagination"
...
import BusPictureView from './BusPictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'busId',
          visible: true,
          scopedSlots: { customRender: 'busId' },
          sorter: true
          //sorter: (a, b) => a.busId - b.busId,
          //sorter: (a, b) => a.busId.localeCompare(b.busId),
        },
        {
          title: 'License Plate',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} buss`,
      },
      ...
      sortBy: 'busId',           // Column to sort by
  ...
  methods: {
    ...
    async renderBussTable() {
      ...
      let bussTableData = [];
      for (let i = 0; i < this.buss.length; i++) {
        bussTableData.push({
          id: i,
          busId: this.buss[i].busId,
          year: this.buss[i].year,
          date: this.buss[i].date,
          competitionId: this.buss[i].competitionId,
          busId: this.buss[i].busId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-buss',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForBussChanged', this.searchQuery);
		//this.renderBussTable();
    },
	...
	async handleAddSubmitted() {
      this.modalBuss = false;

      const currentDate = new Date().getTime();
      this.busToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.busToAdd);
      console.log(jsonData);
      
      const res = await BusService.addBus(jsonData);
