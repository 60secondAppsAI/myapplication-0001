        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Bus Assignments</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalBusAssignments = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalBusAssignments">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add BusAssignment</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="BusAssignmentId" type="text" placeholder="Enter BusAssignmentId" v-model="busAssignmentToAdd.busAssignmentId"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="busAssignments" :row-key="record => record.BusAssignmentId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <BusAssignmentPictureView :busAssignments="busAssignments" />
          </div>
...
import BusAssignmentPictureView from './BusAssignmentPictureView.vue';


const busAssignmentsColumns = [
  "busAssignmentId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    BusAssignmentPictureView
  },

  data() {
    return {
      modalBusAssignments: false,
        isTableView: true,

      columns: [
        {
          title: 'Bus Assignment Id',
		dataIndex: 'busAssignmentId',
          visible: true,
          scopedSlots: { customRender: 'busAssignmentId' },
          sorter: true
          //sorter: (a, b) => a.busAssignmentId - b.busAssignmentId,
          //sorter: (a, b) => a.busAssignmentId.localeCompare(b.busAssignmentId),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} busAssignments`,
      },
      ...
      busAssignmentsTable: {
        columns: [...busAssignmentsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'busAssignmentId',           // Column to sort by
  ...
  methods: {
    ...
    async renderBusAssignmentsTable() {
      ...
      let busAssignmentsTableData = [];
      for (let i = 0; i < this.busAssignments.length; i++) {
        busAssignmentsTableData.push({
          id: i,
          busAssignmentId: this.busAssignments[i].busAssignmentId,
          year: this.busAssignments[i].year,
          date: this.busAssignments[i].date,
          competitionId: this.busAssignments[i].competitionId,
          busAssignmentId: this.busAssignments[i].busAssignmentId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-bus-assignments',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForBusAssignmentsChanged', this.searchQuery);
		//this.renderBusAssignmentsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalBusAssignments = false;

      const currentDate = new Date().getTime();
      this.busAssignmentToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.busAssignmentToAdd);
      console.log(jsonData);
      
      const res = await BusAssignmentService.addBusAssignment(jsonData);
