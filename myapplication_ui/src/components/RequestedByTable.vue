        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Requested Bys</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalRequestedBys = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalRequestedBys">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add RequestedBy</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="RequestedById" type="text" placeholder="Enter RequestedById" v-model="requestedByToAdd.requestedById"></base-input>
  <base-input label="RequestDate" type="text" placeholder="Enter RequestDate" v-model="requestedByToAdd.requestDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="requestedBys" :row-key="record => record.RequestedById" :pagination="pagination"
          ...
          <div v-else>
            ...
            <RequestedByPictureView :requestedBys="requestedBys" />
          </div>
...
import RequestedByPictureView from './RequestedByPictureView.vue';


const requestedBysColumns = [
  "requestedById",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    RequestedByPictureView
  },

  data() {
    return {
      modalRequestedBys: false,
        isTableView: true,
      ...
      columns: [
		...
		dataIndex: 'requestedById',
          visible: true,
          scopedSlots: { customRender: 'requestedById' },
          sorter: true
          //sorter: (a, b) => a.requestedById - b.requestedById,
          //sorter: (a, b) => a.requestedById.localeCompare(b.requestedById),
        },
        {
          title: 'Request Date',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} requestedBys`,
      },
      ...
      requestedBysTable: {
        columns: [...requestedBysColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'requestedById',           // Column to sort by
  ...
  methods: {
    ...
    async renderRequestedBysTable() {
      ...
      let requestedBysTableData = [];
      for (let i = 0; i < this.requestedBys.length; i++) {
        requestedBysTableData.push({
          id: i,
          requestedById: this.requestedBys[i].requestedById,
          year: this.requestedBys[i].year,
          date: this.requestedBys[i].date,
          competitionId: this.requestedBys[i].competitionId,
          requestedById: this.requestedBys[i].requestedById,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-requested-bys',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForRequestedBysChanged', this.searchQuery);
		//this.renderRequestedBysTable();
    },
	...
	async handleAddSubmitted() {
      this.modalRequestedBys = false;

      const currentDate = new Date().getTime();
      this.requestedByToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.requestedByToAdd);
      console.log(jsonData);
      
      const res = await RequestedByService.addRequestedBy(jsonData);
