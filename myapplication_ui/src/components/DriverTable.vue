...
<template>
        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalDrivers = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalDrivers">
                <template slot="header">
  ...
  <base-input label="ContactInformation" type="text" placeholder="Enter ContactInformation" v-model="driverToAdd.contactInformation"></base-input>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="drivers" :row-key="record => record.DriverId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <DriverPictureView :drivers="drivers" />
          </div>
...
import DriverPictureView from './DriverPictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'driverId',
          visible: true,
          scopedSlots: { customRender: 'driverId' },
          sorter: true
      ...
      sortBy: 'driverId',           // Column to sort by
  ...
  methods: {
    ...
    async renderDriversTable() {
      ...
      for (let i = 0; i < this.drivers.length; i++) {
        driversTableData.push({
          id: i,
          driverId: this.drivers[i].driverId,
          year: this.drivers[i].year,
          date: this.drivers[i].date,
          competitionId: this.drivers[i].competitionId,
          driverId: this.drivers[i].driverId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-drivers',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForDriversChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.driverToAdd.created = currentDate;
